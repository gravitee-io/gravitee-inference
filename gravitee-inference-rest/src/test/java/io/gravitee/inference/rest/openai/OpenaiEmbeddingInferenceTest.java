...
public class OpenaiEmbeddingInferenceTest {
  ...
  private static OpenAIEmbeddingConfig getTestConfig()
  ...
  public void setUp() {
    ...
    when(httpRequest.bearerTokenAuthentication(anyString())).thenReturn(
      httpRequest
    );
    when(httpRequest.putHeader(anyString(), anyString())).thenReturn(
      httpRequest
    );
    lenient()
  ...
  public void must_return_embedding() throws URISyntaxException {
    ...
    JsonObject mockResponse = new JsonObject()
      ...
      .put(
        "usage",
        new JsonObject().put("prompt_tokens", 10).put("total_tokens", 10)
      );
    ...
    try (
      MockedStatic<WebClient> webClientMock = Mockito.mockStatic(
        WebClient.class
      )
    ) {
      webClientMock
        .when(() -> WebClient.create(any(Vertx.class)))
        .thenReturn(webClient);

      var client = new OpenaiEmbeddingInference(config, vertx);
      var testObserver = client
        .infer(THE_BIG_BROWN_FOX_JUMPED_OVER_THE_LAZY_DOG)
        .test();
  ...
  public void must_handle_network_failure() throws URISyntaxException {
    ...
    when(httpRequest.rxSendBuffer(any(Buffer.class))).thenReturn(
