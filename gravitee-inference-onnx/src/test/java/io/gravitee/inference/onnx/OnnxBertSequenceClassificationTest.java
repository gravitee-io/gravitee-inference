...
public class OnnxBertSequenceClassificationTest extends OnnxBertBaseTest {
  ...
  private static final String SEQUENCE_TOKENIZER_JSON =
  ...
  private static final OnnxBertResource SEQUENCE_ONNX_BERT_RESOURCE =
    new OnnxBertResource(Path.of(SEQ_MODEL_ONNX), Path.of(SEQ_HF_TOKENIZER));
  ...
  public void must_sequence_classify_and_return_first_highest_score(
    ...
    var sequenceModel = new OnnxBertClassifierModel(
      new OnnxBertConfig(resource, maths, config)
    );
    var positive = sequenceModel
      .infer("I am so happy!")
      .results()
      .stream()
      .toList();
  ...
  public void must_sequence_classify_and_return_first_highest_score_for_both_sentences(
    ...
    var sentiments = sequenceModel.inferAll(
      List.of("I am so happy!", "I am so sad!")
    );
