...
public class OnnxBertEmbeddingInferenceTest extends OnnxBertBaseTest {
  ...
  // ONNX EMBEDDING
  protected static final String SBERT_TOKENIZER_PATH =
    "sbert/all-minilm-l6-v2-q-tokenizer.json";
  protected static final String SBERT_MODEL_ONNX_PATH =
    "sbert/all-minilm-l6-v2-q.onnx";
  ...
  private static final List<OnnxBertConfig> ONNX_BERT_CONFIG_STREAM_CLS =
    List.of(
      new OnnxBertConfig(
        ONNX_BERT_RESOURCE,
        NativeMath.INSTANCE,
      ...
      new OnnxBertConfig(
        ONNX_BERT_RESOURCE,
        MaskAwareSIMDMath.INSTANCE,
        CLS_ONNX_CONFIG
      ),
      new OnnxBertConfig(
        ONNX_BERT_RESOURCE,
        LoopBoundSIMDMath.INSTANCE,
        CLS_ONNX_CONFIG
      )
    );
  ...
  private static final List<OnnxBertConfig> ONNX_BERT_CONFIG_STREAM_MEAN =
    List.of(
      new OnnxBertConfig(
        ONNX_BERT_RESOURCE,
        NativeMath.INSTANCE,
  ...
  public static Stream<
    OnnxBertConfig
  > params_that_must_return_embedding_with_mean() {
    return ONNX_BERT_CONFIG_STREAM_MEAN.stream();
  }

  @Test
  public void embedding_must_be_the_same_with_different_maths() {
    var nativeModel = new OnnxBertEmbeddingModel(
      ONNX_BERT_CONFIG_STREAM_MEAN.get(0)
    );
    var simdModel = new OnnxBertEmbeddingModel(
      ONNX_BERT_CONFIG_STREAM_MEAN.get(1)
    );
    var simdLoopBoundModel = new OnnxBertEmbeddingModel(
      ONNX_BERT_CONFIG_STREAM_MEAN.get(2)
    );

    var resultNative = nativeModel.infer(
      THE_BIG_BROWN_FOX_JUMPED_OVER_THE_LAZY_DOG
    );
    assertEquals(384, resultNative.embedding().length);
    assertEquals(11, resultNative.tokenCount());

    var resultSimdMask = simdModel.infer(
      THE_BIG_BROWN_FOX_JUMPED_OVER_THE_LAZY_DOG
    );
    assertEquals(384, resultSimdMask.embedding().length);
    assertEquals(11, resultSimdMask.tokenCount());

    var resultSimdLoopBound = simdLoopBoundModel.infer(
      THE_BIG_BROWN_FOX_JUMPED_OVER_THE_LAZY_DOG
    );
    ...
    assertArrayEquals(
      resultSimdMask.embedding(),
      resultSimdLoopBound.embedding()
    );
  ...
  public void must_return_embedding(OnnxBertConfig config) {
    ...
    var result = modelInference.infer(
      THE_BIG_BROWN_FOX_JUMPED_OVER_THE_LAZY_DOG
    );
  ...
  public void must_perform_cosine_score(OnnxBertConfig config) {
    var modelInference = new OnnxBertEmbeddingModel(config);
    var result1 = modelInference.infer(
      THE_BIG_BROWN_FOX_JUMPED_OVER_THE_LAZY_DOG
    );
  ...
  public void must_infer_all(OnnxBertConfig config) {
    ...
    var list = modelInference.inferAll(
      List.of(
        THE_BIG_BROWN_FOX_JUMPED_OVER_THE_LAZY_DOG,
        THE_BROWN_FOX_JUMPED_OVER_THE_DOG
      )
    );
  ...
  private static void testResults(
    ...
    assertEquals(
      1.0f,
      gioMath.cosineScore(result1.embedding(), result1.embedding())
    );
    assertEquals(
      1.0f,
      gioMath.cosineScore(result2.embedding(), result2.embedding())
    );

    assertEquals(
      0.91,
      FastMath.ceil(
          gioMath.cosineScore(result1.embedding(), result2.embedding()) * 100
        ) /
        100
    );
  }
